...
*** Ignore: #5 .\pres_bedit.commands.mode.s65:58: .if *** Ignore: 9 .\pres_bedit.command_table.s65:33: .if acornsoft_version

More of #1.

*** Ignore: #6 .\pres_bedit.commands.toggle_insert_overtype.s65:82: .if acornsoft_version

uninline *FX138 call.

(There's only one *FX138 call, so not currently worth it)

*** Ignore (but noteworthy): #7 .\pres_bedit.commands.toggle_insert_overtype.s65:95: .if bet2_version

extra case for run_or_exit: maybe do =*BUTILS=

*** Ignore: #8 .\pres_bedit.commands.toggle_insert_overtype.s65:118: .if bet2_version
...
*** Ignore: #13 .\pres_bedit.command_table.s65:103: .if bet2_version
...
*** Ignore: #16 .\pres_bedit.command_jump_table.s65:161: .if acornsoft_version

CMD_tab_key is now $ca

*** Ignore: #17 .\pres_bedit.command_jump_table.s65:249: .if acornsoft_version

CMD_shift_tab_key is now $ba

*** Ignore: #18 .\pres_bedit.command_jump_table.s65:255: .if bet2_version

New entries for UPDATE and UTILS

*** Ignore: #25 .\pres_bedit.edit_mode_loop.s65:66: .if acornsoft_version

Same as #16

*** Ignore: #27 .\pres_bedit.edit_mode_loop.s65:131: .if acornsoft_version

Same as #16

*** Ignore: #31 .\pres_bedit.edit_mode_loop.s65:189: .if acornsoft_version

Same as #16

*** Ignore: #33 .\pres_bedit.edit_mode_loop.s65:202: .if acornsoft_version

Same as #16

*** Ignore: #50 .\pres_bedit.strings.s65:80: .if acornsoft_version

Help text change.

*** Ignore: #51 .\pres_bedit.strings.s65:115: .if acornsoft_version

Secret credits text change.

*** Ignore: #58 .\pres_bedit.s65:84: .if acornsoft_version

Version number change

*** Ignore: #59 .\pres_bedit.s65:89: .if bet2_version

ROM name change

*** Ignore: #60 .\pres_bedit.s65:95: .if bet2_version

ROM version string change

*** Ignore: #61 .\pres_bedit.s65:105: .if bet2_version

ROM copyright string change

*** Ignore: #66 .\pres_bedit.s65:325: .if acornsoft_version

Looks like a bug fix! Code does ldx #$11 (???) rather than lda #$11
(print char 17)

(...wait, wtf. This code is not part of the Acornsoft ROM! There's no
ldx #$11 anywhere in it. A mystery.)
...
*** Bogus: #53 .\pres_bedit.s65:24:acornsoft_version=false

Irrelevant

*** Bogus: #54 .\pres_bedit.s65:25:bet2_version=false

Irrelevant

*** Bogus: #55 .\pres_bedit.s65:43: .if bet2_version

Irrelevant - set contsants


*** Bogus: #67 .\pres_bedit.s65:373: beq print_version_number

Irrelevant

*** Bogus: #68 .\pres_bedit.s65:378:print_version_number:

Irrelevant

*** Bogus: #69 .\pres_bedit.s65:380:print_version_number_loop:

Irrelevant

*** Bogus: #70 .\pres_bedit.s65:385: bcc print_version_number_loop

Irrelevant

*** Bogus: #71 .\pres_bedit.s65:611: .if bet2_version

Includestuff

*** Bogus: #79 .\pres_bedit.s65:874: .if bet2_version

Includestuff

*** Bogus: #80 .\pres_bedit.s65:1152: .if bet2_version

Includestuff

*** Bogus: #81 .\pres_bedit.s65:1457: .if bet2_version

Includestuff

*** Bogus: #87 .\pres_bedit.s65:1557: .if acornsoft_version

Includestuff

*** Bogus: #88 .\pres_bedit.s65:1674: .if acornsoft_version

Includestuff

*** Bogus: #89 .\pres_bedit.s65:1749: .if bet2_version

Includestuff

*** Bogus: #90 .\pres_bedit.s65:1785: .if bet2_version

End of ROM bits for ABE

*** Bogus: #91 .\pres_bedit.s65:1823: .if acornsoft_version

Includestuff

*** Bogus: #92 .\pres_bedit.s65:1860: .if acornsoft_version
...
*** Done: #9 .\pres_bedit.edit_mode_loop.s65:170: .if bet2_version
*** Done: #20 .\pres_bedit.enter_edit_mode.s65:35: .if bet2_version

Don't do OSBYTE 219 on Electron.

*** Done: #21 .\pres_bedit.enter_edit_mode.s65:42: .if bet2_version

More of #20.

*** Done: #23 .\pres_bedit.edit_mode_loop.s65:20: .if acornsoft_version

Related to #28

*** Done: #24 .\pres_bedit.edit_mode_loop.s65:55: .if bet2_version

Related to #28 I think

*** Done: #28 .\pres_bedit.edit_mode_loop.s65:162: .if bet2_version

Change shift+delete check for Electron - covered by Mince's changes?

(yes - probably! - looks like there's a whole remap table at LABD2. If
it'd be nice to have a version that matches the ABE shortcuts,
whatever they are, this can surely be done with some mods to Mince's
code.)

(There's also some VDU17 stuff I haven't figured out the purpose of.
Need to try this on an actual Electron.)

*** Done: #34 .\pres_bedit.edit_mode_loop.s65:244: .if bet2_version

Handle cursor differently on Electron.

(Mince's changes already deal with this.)

*** Done: #35 .\pres_bedit.edit_mode_loop.s65:252: .if bet2_version

See #34

*** Done: #36 .\pres_bedit.edit_mode_loop.s65:277: .if bet2_version

See #34

*** Done: #37 .\pres_bedit.edit_mode_loop.s65:283: .if bet2_version

See #34

*** Done: #38 .\pres_bedit.edit_mode_loop.s65:291: .if bet2_version

See #34

*** Done: #39 .\pres_bedit.edit_mode_loop.s65:329: .if bet2_version

See #34

*** Done: #40 .\pres_bedit.edit_mode_loop.s65:335: .if acornsoft_version
...
*** Done: #82 .\pres_bedit.s65:1487: .if bet2_version

Don't do OSBYTE 219 on Electron

*** Done: #83 .\pres_bedit.s65:1494: .if bet2_version
...
Change to error message display.
...
Call =call_utils_rom= from bedit_bet2
...
Call =call_edit_rom= from butils_bet2

Ends up calling =handle_butils_to_bedit_call= in bedit_bet2
...
Some of these just get right in there and fiddle about. In the 2nd
processor case, no good, because this is all running from the service
entry in the host. And in the Master case, also potentially no good,
because it's got no qualms about assuming the memory layout.

 =*BZ= and =*BR=, going via the language entry point could be an
option? But seamlessly returning to BASIC afterwards isn't possible,
and the OS prints a bunch of stuff out en route.

But if I've put up with it for those two commands, maybe others would
put up with it for these as well?

Another option (and maybe BZ and BR should be doing this already?)
would be to detect the (very common) host processor case, handle the
case like a normal * command via the service entry point, and get
tidier output. Resort to the language entry point only if absolutely
necessary.
...
Would need to be turned into the language entry point.

*** BLIST

List BASIC file.

OK - takes a file to list, does an osfind to open it and goes
bytewise. So no problem.
...
But there's no obvious Master case - so presumably it got stripped out
at some point? It just always goes right in and loads from $0b00,x...

OK in OS 1.20/OS 2.00/Elk OS 1.00, even with 2nd processor. Key
definitions are almays stored on the host .

Master MOS includes *SHOW, so maybe not super important. Though *SHOW
only shows one key in earlier versions, so not entirely irrelevant.
Needs a version check of some kind in any case.

*** MOVE

Copy memory, with special case for BASIC programs.

Can just refuse to do this if 2nd processor active? (Maybe allow the
general memory copy, but reject the BASIC case? That's probably not
going to be terribly useful generally, though.)

*** ROMID

Read sideways ROM into main memory?

No need for this these days.

*** SPACE

Print some info: HIMEM, LOMEM, TOP, PAGE - and also VARTOP, otherwise
inaccessible.

Needs handling specially in the copro case. Can pull the bytes out of
its memory, if it looks like BASIC is the active language?
...
Ugh... scrapes memory, inserts stuff in keyboard buffer to get BASIC
to do the printing, then inserts a special *VLIST to get the thing to
carry on.

This can probably be reproduced. Still easiest just to say it's never
going to work in the 2nd processor...
